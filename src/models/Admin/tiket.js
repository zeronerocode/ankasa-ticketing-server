...
const tiketModul = {
    getFlight: (data) =>{
        return pool.query('SELECT flights.id, flights.airline_id, airlines.name, flights.departure_city, flights.arrival_city, flights.departure_time, flights.arrival_time, flights.code, flights.class, flights.departure_date, flights.transit, flights.lugage, flights.meal, flights.wifi, flights.gate, flights.terminal, flights.price, flights.stock, flights.duration, flights.is_active, flights.created_at, flights.updated_at FROM flights LEFT JOIN airlines ON flights.airline_id = airlines.id LIMIT $1 OFFSET $2', [data.limit, data.offset])
    },
    detailFlight: (data) =>{
        return pool.query('SELECT flights.id, flights.airline_id, airlines.name, flights.departure_city, flights.arrival_city, flights.departure_time, flights.arrival_time, flights.code, flights.class, flights.departure_date, flights.transit, flights.lugage, flights.meal, flights.wifi, flights.gate, flights.terminal, flights.price, flights.stock, flights.duration, flights.is_active, flights.created_at, flights.updated_at FROM flights LEFT JOIN airlines ON flights.airline_id = airlines.id WHERE flights.id = $1', [data])
    },
